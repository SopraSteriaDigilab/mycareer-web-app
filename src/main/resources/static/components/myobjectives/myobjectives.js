...
function initMyObjectives(){
    ...
    $('#delete').click(function(){ deleteObjective(ADLoginID, $("#delete-id").text(), $("#deleteTitle").text(), $("#deletingText").val()); });
    
    //onclick to complete objective
    $('#submit-completed-status-note').click(function(){ 
    	editObjectiveProgressOnDB(ADLoginID, $("#complete-id").text(), $("#complete-status").text(), $("#completedTitle").text(), $('#completedText').val());
    });
}
...
function addObjectiveToDB(userId, objTitle, objText, objDate) {
	var success = function(response){
        ...
        addObjectiveToList(Id, objTitle, objText, formatDate(objDate), 0, false, ADfullName, timeStampToLongDate(new Date()));
	    showProposedObjTab();
...
function editObjectiveProgressOnDB(userId, objID, objStatus, objTitle, comment){
	var success = function (response){
        ...
        if(objStatus == 2){
        	...
        	var text = (comment === "") ? ADfullName + " has completed Objective '"+ objTitle +"'." : ADfullName + " has completed Objective '"+ objTitle +"'. "+" A comment was added: '"+ comment+"'.";
        	var d = new Date();
...
function deleteObjective(userId, objID, objTitle, comment){
	var success = function(response){
        ...
        var text = (comment === "") ? ADfullName + " has deleted Objective '"+ objTitle +"'." : ADfullName + " has deleted Objective '"+ objTitle +"'. "+" A comment was added: '"+ comment+"'.";
    	var d = new Date();
      	var date = moment(d).format('DD MMM YYYY HH:mm');
      	var classDate = moment(d).format('YYYY-MM-DD');
        addNoteToList(id, "Auto Generated", text, date, classDate, emptyArray, emptyArray);
